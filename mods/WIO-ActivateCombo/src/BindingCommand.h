...
// itself to "No Change". That reset needs both the ini write, which this plugin reads next pass,
// and MCM.SetModSettingInt, which the menu shows this session, because MCM builds its settings
// store once per launch. Only the ini write is correctness-critical.
namespace ARC::BindingCommand
{
	...
	namespace detail
	{
		inline constexpr const char* kModName = "WIO-ActivateCombo";
		inline constexpr const char* kEvent = "ReadyWeapon";
		...
		inline constexpr std::array<Control, 2> kControls{ {
			{ "Gamepad", "iUnbindReadyWeaponGamepad", "sReadyWeaponGamepadStatus:Gamepad",
			  WattzIO::ControlUnbinderAPI::Slot::kGamepad, "$ARC_Dev_Gamepad", "Gamepad" },

			{ "Keyboard", "iUnbindReadyWeaponKeyboard", "sReadyWeaponKeyboardStatus:Keyboard",
			  WattzIO::ControlUnbinderAPI::Slot::kKeyboard, "$ARC_Dev_Keyboard", "Keyboard" },
		} };
		...
		// Pushed values round-trip through an ini, so characters the format treats as structure are
		// neutralised: a ';' would make the whole row vanish on read-back.
		...
		inline void ResetControl(const Control& a_control)
		{
			...
			if (!::WritePrivateProfileStringA(a_control.section, a_control.key, "0",
				...
				REX::WARN(
					"Activate/Reload Combo: could not reset {} in the MCM ini - the command may repeat"sv,
					a_control.key);
			}
		...
		[[nodiscard]] inline std::string StatusText(const Control& a_control)
		{
			...
			if (!api) {
				return WIO::Translations::Localize(
					"$ARC_Status_NeedsUnbinder"sv, "Requires Control Unbinder"sv);
			}

			const auto* described = api->DescribeBinding(kEvent, a_control.slot);
			return described ? std::string{ described } :
							   WIO::Translations::Localize("$ARC_Status_Unknown"sv, "Unknown"sv);
		}
	}
	...
	inline void Run()
	{
		...
		for (const auto& control : detail::kControls) {
			...
			// Reports the binding change only. One key per outcome, in whole sentences, because words
			// translated in isolation do not fit other languages' grammar. {EVENT} is Control Unbinder's.
			...
			case Api::Result::kSuccess:
				show(command == Command::kUnbind ?
						 WIO::Translations::Localize("$ARC_Note_BindUnbound"sv,
							 "Control Unbinder: {EVENT} unbound ({DEVICE})"sv, args) :
						 WIO::Translations::Localize("$ARC_Note_BindRestored"sv,
							 "Control Unbinder: {EVENT} restored ({DEVICE})"sv, args));
				break;
			case Api::Result::kAlreadyInState:
				break;
			case Api::Result::kNotApplicable:
				show(WIO::Translations::Localize("$ARC_Note_BindNoBinding"sv,
					"Control Unbinder: {EVENT} - no {DEVICE} binding"sv, args));
				break;
			default:
				show(command == Command::kUnbind ?
						 WIO::Translations::Localize("$ARC_Note_BindUnbindFailed"sv,
							 "Control Unbinder: {EVENT} unbind failed ({DEVICE})"sv, args) :
						 WIO::Translations::Localize("$ARC_Note_BindRebindFailed"sv,
							 "Control Unbinder: {EVENT} rebind failed ({DEVICE})"sv, args));
				break;
			}

			REX::INFO("Activate/Reload Combo: command {} on {} - {}"sv, action, control.label,
