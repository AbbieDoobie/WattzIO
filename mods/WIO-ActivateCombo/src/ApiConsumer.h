...
namespace ARC::ApiConsumer
{
	...
	inline void Acquire()
	{
		...
		if (!getApi) {
			REX::WARN("Activate/Reload Combo: {} is loaded but does not export {}"sv,
		...
		if (api->version != Api::kVersion) {
			REX::WARN(
				"Activate/Reload Combo: Control Unbinder API v{} does not match the v{} this mod was "
		...
		detail::g_api = api;
		REX::INFO("Activate/Reload Combo: Control Unbinder API v{} acquired"sv, api->version);
	}
	...
	inline void WarnUnavailable()
	{
		...
		const auto msg = WIO::Translations::Localize(
			"$ARC_Note_UnbinderMissing"sv, "Activate/Reload Combo: Control Unbinder not installed"sv);
		RE::SendHUDMessage::ShowHUDMessage(msg.c_str(), nullptr, true, true);
		REX::WARN(
			"Activate/Reload Combo: a binding option is set but WIO-ControlUnbinder is not installed"sv);
	}
}
