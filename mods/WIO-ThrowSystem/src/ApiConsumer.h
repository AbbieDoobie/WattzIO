...
namespace TSO::ApiConsumer
{
	...
	inline void Acquire()
	{
		...
		if (!getApi) {
			REX::WARN("Throwing System Overhaul: {} is loaded but does not export {}"sv,
		...
		if (api->version != Api::kVersion) {
			REX::WARN(
				"Throwing System Overhaul: Control Unbinder API v{} does not match the v{} this mod was "
		...
		detail::g_api = api;
		REX::INFO("Throwing System Overhaul: Control Unbinder API v{} acquired"sv, api->version);
	}
	...
	inline void WarnUnavailable()
	{
		...
		const auto msg = WIO::Translations::Localize(
			"$TSO_Note_UnbinderMissing"sv,
			"Throwing System Overhaul: Control Unbinder not installed"sv);
		RE::SendHUDMessage::ShowHUDMessage(msg.c_str(), nullptr, true, true);
		REX::WARN(
			"Throwing System Overhaul: a binding option is set but WIO-ControlUnbinder is not installed"sv);
	}
}
